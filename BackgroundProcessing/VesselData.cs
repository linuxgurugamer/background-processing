...
namespace BackgroundProcessing {
	...
	class VesselData {
		...
		private static bool HasResourceGenerationData(
            ...
            if (m != null && m.moduleName == "ModuleResourceConverter")
            {
                return ResourceHandlers.ResourceConverter.HasResourceGenerationData(m, s, resourceData, interestingResources);
            }
            if (m != null && m.moduleName == "ModuleBackgroundProcessing")
            {
                return ResourceHandlers.HasModuleBackgroundProcessing.HasBackgroundProcessingData(m, s, resourceData, interestingResources);
            }
		...
		private static List<ResourceModuleHandler> GetResourceGenerationData(
            ...
            if (m != null && m.moduleName == "ModuleResourceConverter")
            {
                ret.AddRange(ResourceHandlers.ResourceConverter.GetResourceGenerationData(m, part, resourceData, interestingResources));
            }
            ...
            if (m != null && m.moduleName == "ModuleBackgroundProcessing")
            {
                ret.AddRange(ResourceHandlers.HasModuleBackgroundProcessing.GetBackgroundProcessingData(m, part, resourceData, interestingResources));
            }
		...
		public static VesselData GetVesselData(
			...
			if (v.protoVessel != null) {
				foreach (ProtoPartSnapshot p in v.protoVessel.protoPartSnapshots) {
					...
					if (part == null) {
						BackgroundProcessing.Debug("Couldn't find PartPrefab for part " + p.partName, DebugLevel.WARNING);
						continue;
					}
					...
					for (int i = 0; i < p.modules.Count; ++i) {
						if (p.modules[i].moduleName == null) {
							BackgroundProcessing.Debug("Null moduleName for module " + i + "/" + p.modules.Count, DebugLevel.WARNING);
							BackgroundProcessing.Debug("Module values: " + p.modules[i].moduleValues, DebugLevel.WARNING);
							continue;
						}
						...
						if (j >= part.Modules.Count || part.Modules[j].moduleName != p.modules[i].moduleName) {
							if (j < part.Modules.Count) {
								BackgroundProcessing.Debug("Expected " + p.modules[i].moduleName + " at index " + i + ", got " + part.Modules[j].moduleName, DebugLevel.WARNING);

								for (j = i; j < part.Modules.Count; ++j) {
									if (part.Modules[j].moduleName == p.modules[i].moduleName) {
										BackgroundProcessing.Debug("Found " + p.modules[i].moduleName + " at index " + j, DebugLevel.WARNING);
										break;
									}
								}
							}
						...
						if (j < part.Modules.Count) {
							...
							BackgroundProcessing.Debug("Ran out of modules before finding module " + p.modules[i].moduleName, DebugLevel.WARNING);
					...
					foreach (ProtoPartResourceSnapshot r in p.resources) {
						if (r.resourceName == null) {
							BackgroundProcessing.Debug("Null resourceName.", DebugLevel.WARNING);
							continue;
						}

						if (!r.flowState) { continue; }
